...
module ICloud.Http.CliSpec (spec) where

import Network.ICloud.Http.Cli (CommonOpts (..), commonOptsParser)
import Options.Applicative (ParserResult (..), defaultPrefs, execParserPure, fullDesc, info, renderFailure)
import Test.Hspec


spec :: Spec
spec = describe "Network.ICloud.Http.Cli.commonOptsParser" $ do
