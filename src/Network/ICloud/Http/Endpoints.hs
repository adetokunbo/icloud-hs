...
Defines 'Endpoints', a datatype representing specific HTTP endpoints used at the
base of URLs in the ICloud api, along with various functions that are used with
@Endpoints@ construct API requests
-}
module Network.ICloud.Http.Endpoints
  ( -- * choice of ICloud domain
    Realm

    -- ** select @Endpoints@ by @Realm@
  , realmEndpoints

    -- * provide base URLs used by API requests
  , Endpoints (..)

    -- ** make endpoint-specific @Requests@
  , accountLoginBase
  , listDevices
  , validateBase
